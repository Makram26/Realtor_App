...
export default function EditTask({ navigation, route }) {
    ...
    console.log("items[edit][tasks]: ", items)
    ...
    // Priority States
    const [btnHighColor, setBtnHighColor] = useState(priorty == "High" ? true : false)
    const [btnMediumColor, setBtnMediumColor] = useState(priorty == "Medium" ? true : false)
    const [btnLowColor, setBtnLowColor] = useState(priorty == "Low" ? true : false)

    // Status States
    const [btnNewColor, setBtnNewColor] = useState(status == "New" ? true : false)
    const [btnDoneColor, setBtnDoneColor] = useState(status == "Done" ? true : false)
    const [btnLateColor, setBtnLateColor] = useState(status == "Late" ? true : false)
    const [btnPendingColor, setBtnPendingColor] = useState(status == "Pending" ? true : false)

    // Subject States
    const [btnMeetingColor, setBtnMeetingColor] = useState(subject == "Meeting" ? true : false)
    const [btnCallColor, setBtnCallColor] = useState(subject == "Call" ? true : false)
    const [btnToDoColor, setBtnToDoColor] = useState(subject == "ToDo" ? true : false)
    const [btnSiteVisitColor, setBtnSiteVisitColor] = useState(subject == "SiteVisit" ? true : false)

    // date states
    let today = new Date().toLocaleDateString()
    // console.log(today)
    const [dates, setDate] = useState(new Date(date));
    ...
    const sendNotification = async () => {
        if (oneDeviceToken !== "") {
            try {
                let res = await NotificationApi("Task Assign", `${user.displayName} assign a new Task to you`, oneDeviceToken)
                console.log("notification responce: ", res)
                if (res.msg === "Successfully") {
                    navigation.pop(2)
                }
                else {
                    alert("Notication not Send But Task Updated")
                    navigation.pop(2)
                }
            } catch (error) {
                alert("Error in Notification But task has been Updated")
                navigation.pop(2)
            }
        }
        else {
    ...
    const onChange = (event, selectedDate) => {
        console.log("date wise ", selectedDate)
        if (event.type === "set") {
    ...
    // console.log("inventories inventories inventories",leads.id)
    // leadName
    ...
    const updateTaskHandler = async () => {
        ...
        try {
            await firestore()
                ...
                .doc(id)
                .update({
                    subject: btnMeetingColor ? "Meeting" : btnCallColor ? "Call" : btnToDoColor ? "ToDo" : "SiteVisit",
                    title: titles !== "" ? titles : title,
                    date: newDate != "" ? newDate : date,
                    status: newDate === "" ? status : newDate < today ? "Late" : "New",
                    time: newTime !== "" ? newTime : time,
                    // btnNewColor ? "New" : btnDoneColor ? "Done" : btnLateColor ? "Late" : "Pending",
                    priorty: btnHighColor ? "High" : btnMediumColor ? "Medium" : "Low",
                    note: notes && notes !== note ? notes : note,
                    inventory: inventories,
                    lead: leads,
                    inventoryID: inventories ? inventories.id : null,
                    leadID: leads ? leads.id : null,
                    leadName: leads ? leads.leadName : null,
        ...
        <ScrollView keyboardShouldPersistTaps="always" contentContainerStyle={{ flexGrow: 1, backgroundColor: "#FCFCFC" }}>
            ...
            <View style={HeaderStyle.mainContainer}>
                ...
                <View style={HeaderStyle.HeaderTextContainer}>
                    <Text style={HeaderStyle.HeaderText}>Edit Task</Text>
                    <Image
            ...
            <View style={styles.body}>
...

                {
                    showTitle ?
                        <TextInput
                            ...
                            value={titles}
                            onChangeText={(value) => setTitles(value)}
                        />
                        :
                        <Text style={{
                            fontSize: 15,
                ...
                {/* <Text style={{ fontSize: 16, fontWeight: '700', color: '#1A1E25', marginTop: 10 }}>Due Date</Text>
                <Text>{date}</Text> */}
                ...
                <View style={{
                    ...
                    <TouchableOpacity
                        ...
                        onPress={() => { setShowDate(!showDate), setNewDate("") }}
                    >
                        <Ionicons
                            name='pencil-outline'
                            size={20}
                    ...
                    showDate ?
                        <TouchableOpacity onPress={() => showMode('date')} style={{ ...styles.datePickerContainer }}>
                            <Text style={{
                                ...
                                color: 'gray',
                                // marginTop: 10
                            }}>{newDate == "" ? "select date" : newDate}</Text>
                ...
                <View style={{
                    ...
                    <Text style={{ fontSize: 16, fontWeight: '700', color: '#1A1E25', marginTop: 10 }}>Due Time</Text>
                    <TouchableOpacity
                        style={{
                            width: '20%',
                            // borderColor:'red',
                        ...
                        onPress={() => { setShowTime(!showTime), setNewTime("") }}
                    >
                        <Ionicons
                            name='pencil-outline'
                            size={20}
                    ...
                    showTime ?
                        <TouchableOpacity onPress={() => showMode('time')} style={{ ...styles.datePickerContainer }}>
                            <Text style={{
                                ...
                                color: 'gray',
                                // marginTop: 10
                            }}>{newTime == "" ? "select Timr" : newTime}</Text>
                ...
                <View style={{
                    ...
                    <Text style={{ fontSize: 16, fontWeight: '700', color: '#1A1E25', marginTop: 10 }}>Note</Text>
                    <TouchableOpacity
                        style={{
                            width: '20%',
                            // borderColor:'red',
                    ...
                    userType == 0 ?
                        <>
                            <View style={{ flexDirection: "row", justifyContent: "space-between", }}>
                                <View style={{ flex: 0.6 }}>
                                    <Text style={{ fontSize: 16, fontWeight: '700', color: '#1A1E25', marginTop: 10 }}>Sale Person</Text>
                                </View>
                            </View>
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: "center", }}>
                                ...
                                <View style={[styles.pickerStyle, { marginTop: 10 }]}>
                                    <Picker
                                        selectedValue={value}
