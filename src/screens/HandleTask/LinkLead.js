...
export default function LinkLead({ navigation }) {
    ...
    const ShowLeads = async () => {
        ...
        let tempToLet = []
        setLoading(true)
        const response = await LeadApi.getLeads(user.uid)
            ...
            <View style={{ width: "92%", flexDirection: 'row', justifyContent: "space-between", margin: 15, alignSelf: "center" }}>
                <View style={{ flex: 1, flexDirection: "row", justifyContent: "space-between", marginTop: 15, }}>
                    ...
                    <TouchableOpacity onPress={() => ColorChangeRent()}
                        ...
                        <Text style={{ color: textRent, fontSize: 12 }}>For Rent</Text>
                    </TouchableOpacity>
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => (
                            <LinkLeadCard
                                ...
                                navigation={() => navigation.navigate("CreateTask", item)}
                            />
                        )}
                    />
                    :
